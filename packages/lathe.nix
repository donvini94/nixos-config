...
buildGoModule (finalAttrs: {
  ...
  # Without Version the UI and `lathe version` report "dev", and the update check
  # compares against a version that never matches.
  ldflags = [
    "-s"
    "-w"
  ...
  # `skills install` without --user writes into ./.claude/skills and this source tree
  # already has one, so it runs in a throwaway dir.
  postInstall = ''
    stage=$(mktemp -d)
