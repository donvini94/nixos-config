...
All four addresses are loopback-only: they are reachable from `dracula`, not the LAN or
internet.
...
Every application container declared in this repository uses an upstream rolling tag.
