{ ... }:
# Keys are named once and referenced per account. Every account below keeps
# exactly the key set it had before: the lists are assembled from these names,
# not widened. Adding a person means adding one binding and naming it on the
# accounts they are actually meant to reach.
let
  vincenzoNixos = "ssh-rsa  vincenzo@nixos";
  vincenzoDracula = "ssh-rsa  vincenzo@dracula";
  marius = "ssh-rsa  Marius";
  kyrill = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJtziDwyaqKfBPL1dDEM6kMdA+KTL+d0810PzAbOsWHn kyrill@kyrill-ThinkPad-T495";

  vincenzoKeys = [
    vincenzoNixos
    vincenzoDracula
  ];
  adminKeys = vincenzoKeys ++ [
    marius
    kyrill
  ];
in
{
  users.users = {
    vincenzo = {
      isNormalUser = true;
      extraGroups = [
        "wheel"
        "docker"
      ];
      openssh.authorizedKeys.keys = vincenzoKeys;
    };
    nix = {
      isNormalUser = true;
      extraGroups = [
        "wheel"
        "docker"
      ];
      openssh.authorizedKeys.keys = adminKeys;
    };
    marius = {
      isNormalUser = true;
      extraGroups = [ "wheel" ];
      openssh.authorizedKeys.keys = [ marius ];
    };
    kyrill = {
      isNormalUser = true;
      extraGroups = [ "wheel" ];
      openssh.authorizedKeys.keys = [ kyrill ];
    };
    jellyfin = {
      extraGroups = [
        "wheel"
        "docker"
      ];
      openssh.authorizedKeys.keys = vincenzoKeys ++ [ kyrill ];
    };
  };
}
