...
The Emacs daemon is launched by launchd (the Homebrew =emacs-plus= LaunchAgent),
which provides only the bare macOS PATH (=/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin=).
That omits =/opt/homebrew/bin=, so ~executable-find~ can't see Homebrew tools
(pngpaste, fish, rust-analyzer, tex, …) — e.g. =org-download-clipboard= fails with
"Please install the pngpaste program". Pull the real environment from the login shell.

Must run *before* any load-time ~(executable-find …)~ calls below (e.g. the =fish=
lookups in Defaults). Gated on daemon/GUI so batch invocations (=doom sync/doctor=)
don't pay the shell-probe cost.
...
Stack: mbsync (IMAP sync) → ~/Maildir → notmuch (Xapian index) → notmuch.el (Emacs UI) → msmtp (send).
Tagging pipeline lives in ~~/.local/bin/mail-sync~ — a pure-shell script that
runs ~mbsync -a → notmuch new → notmuch tag (one invocation per rule)~. See
that script for the rule chain and order; rules are intentionally explicit
rather than declarative-via-framework.

Tag taxonomy and triage workflow are documented in
~~/org/roam/reference/<date>-notmuch.org~ — that is the source of truth for what
each tag means. This section only covers the Emacs-side configuration.

** Identity and sending (msmtp)
The notmuch sending stack uses the same message.el + msmtp pipeline that mu4e
used. ~message-sendmail-envelope-from 'header~ makes msmtp pick the account
from the From: header, so identity switching is automatic.
...
** Saved searches
The notmuch ~hello~ buffer (=SPC o m=) shows these as a clickable list with
live counts. ~tag:inbox~ is the decision queue — everything else is a
specialized view.

The first two entries (=Owed reply=, =Waiting=) are the dashboard core:
they're sourced from computed tags written by ~mail-sync~
(see ~~/.local/bin/notmuch-compute-state~) and answer the only two
questions that actually matter — /am I the bottleneck/ and /are they?/
...
** Aging colors
Threads in =tag:owed/reply= and =tag:s/waiting= are aged into four buckets
by ~notmuch-compute-state~. Faces below colorize the search buffer line
based on which bucket a thread falls into. The aging tag itself is hidden
from the tag pill (it'd just be noise on every line) but the color carries
the signal.
...
** Hello dashboard
The stock ~notmuch-hello~ is a sparse grid of saved-search counts and a tag
list. We replace it with a denser, structured dashboard:

1. *Header* — total messages indexed, last sync time
2. *Action queues* — owed-reply + waiting with aging breakdown (red/orange/yellow/green counts), each number clickable
3. *Today's flow* — per-tag delta vs the 03:00 baseline (~~/org/notmuch-counts/YYYY-MM-DD.csv~)
4. *Saved searches* — the standard grid (slightly compressed)
5. *Search* — search prompt
6. *All tags* — the bottom tag list

Counts are computed via one batched ~notmuch count --batch~ per section
(via ~notmuch-hello-query-counts~), so refresh is cheap.
...
** Tag keybindings
All tag operations go through one mechanism: ~K~ → ~notmuch-tag-jump~ → a
transient menu populated from ~notmuch-tagging-keys~. Lower-case key adds
the tag, upper-case removes. Press =K= in any notmuch buffer, then the
letter.

Why one mechanism: simpler than a mix of toggle-helpers + localleader +
native menu. Two keypresses per tag operation, but it's a *consistent* two
keypresses with the transient visibly listing what each letter does. No
muscle memory burden for rare tags.

There's a load-order subtlety: Doom binds ~K~ globally in evil
normal+visual states to ~+lookup/documentation~ (see
~modules/editor/evil/config.el:556~), and that global state-map binding
overrides ~evil-collection-notmuch~'s mode-local ~K~. We rebind it
explicitly in the notmuch mode-maps to win the race.

Other useful evil-collection bindings already in place: ~a~ archive,
~J~ jump-to-saved-search, ~+~/~-~ ad-hoc tag, ~s~ new search, ~c c~ compose.
...
** consult-notmuch — async live-preview search
Type-as-you-go search across the full index with Consult preview. Use for
ad-hoc lookups when a saved search doesn't fit.
...
* Native Compilation Toolchain Fix
This Emacs is the =emacs-plus-app= Homebrew *cask* (a prebuilt binary) that
bundles its own libgccjit. When Homebrew's =gcc= rolls to a new major version
(e.g. 15 -> 16), the bundled libgccjit can no longer find gcc's runtime libs
(=libemutls_w.a=, =libgcc=, crt objects) and native compilation breaks with
=ld: library 'emutls_w' not found / error invoking gcc driver=.

The *primary* fix is shell-level: =~/.config/fish/conf.d/emacs-libgccjit-fix.fish=
exports =LIBRARY_PATH= before =doom=/Emacs launch. That is required because subr
trampolines compile in child processes during Doom *core boot*, before any user
Lisp loads — too early for a Doom-config hook to help.

The =native-comp-driver-fix.el= loaded below is only a safety net for a
*Dock-launched* GUI Emacs (no inherited shell env): its =setenv= lets
post-startup lazy/async native compiles inherit =LIBRARY_PATH=. Globbed so it
self-heals across gcc major bumps; a no-op on non-Homebrew hosts.
