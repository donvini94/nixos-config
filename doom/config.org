...
Stack: mbsync (IMAP sync) → ~/Maildir → mu (Xapian index) → mu4e (Emacs UI) → msmtp (send).
Two accounts: ~personal~ (iCloud, active) and ~work~ (amiconsult.de M365, pending IT consent).

Sync is driven by a launchd agent every 5 minutes — mu4e itself does not fetch.
~U~ in mu4e re-indexes from disk (cheap); ~mbsync -a~ in a terminal force-pulls.

** Make Homebrew's mu4e discoverable
On macOS, Homebrew installs mu4e under the ~mu~ formula's prefix instead of
the default Emacs site-lisp dir. Emacs cannot find it without this hint.
Must run BEFORE any ~(after! mu4e ...)~ block — if mu4e fails to load,
nothing inside ~after!~ ever fires.

The path is derived from the active Homebrew cellar so it survives ~brew upgrade mu~.
...
** General
Doom autoloads mu4e on first invocation (~SPC o m~). We set the binary path
explicitly so it works regardless of ~exec-path~ at startup.
...
A ~mu4e-context~ binds an identity to a maildir tree, signature, sent-folder, and
SMTP account. Switching context (=SPC m c= or auto-pick on reply) changes
~user-mail-address~, ~smtpmail-default-smtp-server~, etc. simultaneously.

Doom's ~set-email-account!~ macro wraps the underlying ~make-mu4e-context~.
...
** Sending mail (msmtp)
mu4e composes the message, then pipes it to ~msmtp -a <account>~. msmtp reads
~/.msmtprc, selects the account, and handles TLS + auth. Logs land in
~/Library/Logs/msmtp.log — first place to grep when sending breaks.

The key trick: ~message-sendmail-envelope-from 'header~ tells msmtp to derive the
account from the ~From:~ header, which we control via mu4e-context. So context
switching automatically picks the right SMTP account.
...
** Headers view (columns + sorting)
Custom column layout inspired by James Endres Howell's post on mu4e headers
sorting. ~:human-date~ shows relative dates ("3d", "Mon"), ~:flags~ shows
read/replied/attachment glyphs.
...
** Maildir shortcuts
~J~ in mu4e jumps to a folder by single keypress.
...
Bookmarks are the IT consultant's killer feature: queries you run constantly
get a single-key shortcut. Press ~b~ in mu4e to see the list.

These are starter defaults — replace with your real triage queries once you
know what you actually look at every day.
...
** consult-mu — async live-preview search
The piece from Armin Darvish's blog post. Replaces ~mu4e-headers-search~ with
a Consult interface: type a query, see results stream in with live preview,
Embark gives you actions (reply, archive, copy link).

Trigger via ~M-x consult-mu~ (we also bind ~SPC s m~).
...
** HTML rendering — mu4e-views (optional)
xwidget-webkit gives real WebKit rendering inside Emacs. Only works if your
Emacs was compiled with xwidgets support. Test by running ~M-x xwidget-webkit-browse-url~;
if it errors, leave this disabled and use the default shr renderer.
...
** Keybindings (under SPC m for "mail")
Doom binds =SPC o m= to open mu4e. The leader prefix ~SPC m~ is reserved per-major-mode,
but in non-mail buffers we hijack a sub-namespace for cross-buffer mail actions.
