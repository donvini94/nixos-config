...
;; Set before magit loads — the version check runs during loading, so `after!' fires too
;; late. Resolved with `executable-find' rather than a hardcoded prefix: this used to be
;; "/opt/homebrew/bin/git", and when AC-0137 moved to nix-darwin that formula was
;; uninstalled, leaving magit pointing at a path that no longer existed. git now comes
;; from the per-user nix profile there. The `exec-path-from-shell' block above has already
;; imported the login shell's PATH by this point, which is what makes the lookup see it.
(when (eq system-type 'darwin)
...
(after! org
  ...
  (setq org-agenda-files
        (append (directory-files-recursively (concat org-directory "gtd/") "\\.org$")
                ...
                ;; Auto-generated by ~/.local/bin/notmuch-emit-followups
                ;; (one entry per owed-reply or waiting thread).
                (let ((mf (concat org-directory "mail-followups.org")))
                  (and (file-exists-p mf) (list mf))))))
...
(after! org
  ...
  (setq org-agenda-custom-commands
        '(("o" "Dashboard"
          ...
          ;; Mail follow-ups — owed-reply + waiting queues, sourced from
          ;; the auto-generated ~/org/mail-followups.org. Filters strictly
          ;; to entries from that file so it doesn't pull in your real TODOs.
          ("M" "Mail follow-ups"
           ((alltodo ""
                     ((org-agenda-overriding-header "")
                      (org-agenda-files
                       (list (concat org-directory "mail-followups.org")))
                      (org-super-agenda-groups
                       '((:name " Owed reply (act now) "
                          :and (:tag "owed_reply" :priority "A"))
                         (:name " Owed reply "
                          :tag "owed_reply")
                         (:name " Waiting >7d (nudge) "
                          :and (:tag "s_waiting" :priority "A"))
                         (:name " Waiting 3–7d "
                          :and (:tag "s_waiting" :priority "B"))
                         (:name " Waiting <3d "
                          :tag "s_waiting")
                         (:discard (:anything t)))))))))))
...
;; [[file:config.org::*Identity and sending (msmtp)][Identity and sending (msmtp):1]]
(after! notmuch
  (setq user-mail-address      "vincenzo.pace94@icloud.com"
        user-full-name         "Vincenzo Pace"

        ;; Sending via msmtp (logs at ~/Library/Logs/msmtp.log when things break).
        ...
        ;; Sent mail: write a copy via Fcc into the personal/Sent maildir.
        ;; The mail-sync pipeline tags it +sent and keeps it out of :inbox.
        notmuch-fcc-dirs '(("vincenzo.pace94@icloud.com" . "personal/Sent Messages")
                           ;; Uncomment once work account is connected.
                           ;; Path must match the Channel name in ~/.mbsyncrc
                           ;; (currently `work` → ~/Maildir/work/), and the
                           ;; folder name M365 actually uses (`Sent Items`).
                           ;; ("pace@amiconsult.de"       . "work/Sent Items")
                           )

        ;; Identities for compose. On REPLY, notmuch picks From automatically
        ;; based on which of these addresses received the original mail.
        ;; On NEW compose, prompts (because notmuch-always-prompt-for-sender = t).
        ;; msmtp's --read-envelope-from then routes the actual send.
        notmuch-identities '("Vincenzo Pace <vincenzo.pace94@icloud.com>"
                             "Vincenzo Pace <pace@amiconsult.de>")
        notmuch-always-prompt-for-sender t))
;; Identity and sending (msmtp):1 ends here

;; [[file:config.org::*General behavior][General behavior:1]]
(after! notmuch
  (setq notmuch-search-oldest-first nil          ; newest first in search results
        notmuch-show-logo nil                    ; saves screen real estate
        notmuch-show-all-tags-list t             ; show every tag on a message, not just custom
        notmuch-archive-tags '("-inbox")         ; what `a` (archive) does — strip :inbox, no extras
        notmuch-show-empty-saved-searches nil    ; hide zero-count searches from the hello buffer

        ;; Reply quoting
        notmuch-mua-cite-function #'message-cite-original-without-signature
        notmuch-mua-reply-insert-header-p-function #'notmuch-show-reply-insert-header-p-never

        ;; Mark thread as read when you view it.
        notmuch-show-mark-read-tags '("-unread")))

;; Override Doom's notmuch popup rules so reading happens in the full frame.
;; Doom's =email/notmuch= module ships these popup rules (config.el):
;;   (set-popup-rule! "^\\*notmuch"        :ignore t)                       ;; broad — already full-frame
;;   (set-popup-rule! "^\\*notmuch-hello"  :side 'left :size 30 :ttl 0)     ;; narrow left side-window
;;   (set-popup-rule! "^\\*subject:"       :size 0.6  :ttl 0)               ;; 60% popup for show buffers
;; The narrower rules win, which is why notmuch-hello opens cramped on the
;; left and reading a thread pops a half-screen overlay. Override both to
;; :ignore so they obey the broad rule above.
;; (Doom uses a defadvice to rename the show buffer from "*notmuch-show:...*"
;; to "*subject:...*", hence the subject: pattern.)
(after! notmuch
  (set-popup-rule! "^\\*notmuch-hello" :ignore t)
  (set-popup-rule! "^\\*subject:"      :ignore t))

;; Empty search query → search all mail.
;;
;; The hook is `notmuch-read-query` — the function that runs the actual
;; prompt — NOT `notmuch-search`. Reason: when `s` is pressed, the
;; interactive form passes query=nil; the function body then calls
;; `notmuch-read-query` to ask the user. If we advised notmuch-search and
;; substituted "*" for nil, we'd bypass the prompt entirely (and `s` would
;; immediately run "*" with no way to actually type a query). Advising
;; the read function means: prompt always runs; only an *empty submission*
;; gets translated to "*".
(defun my/notmuch-read-query-default-all (orig-fn &rest args)
  (let ((result (apply orig-fn args)))
    (if (and (stringp result) (string-empty-p (string-trim result)))
        (progn (message "[notmuch] empty query → searching all mail (*)") "*")
      result)))
(advice-add 'notmuch-read-query :around #'my/notmuch-read-query-default-all)
;; General behavior:1 ends here

;; [[file:config.org::*Saved searches][Saved searches:1]]
(after! notmuch
  (setq notmuch-saved-searches
        '(;; --- Dashboard core ---------------------------------------------
          (:name "↩ Owed reply"   :query "tag:owed/reply"                     :key "o" :sort-order oldest-first)
          (:name "⏳ Waiting"      :query "tag:s/waiting"                      :key "w" :sort-order oldest-first)
          ;; --- Daily queues ----------------------------------------------
          (:name "Inbox"          :query "tag:inbox"                          :key "i" :sort-order newest-first)
          (:name "Reading (1w)"   :query "tag:reading AND date:7d.."          :key "r" :sort-order newest-first)
          (:name "Follow-up"      :query "tag:s/follow-up"                    :key "f" :sort-order oldest-first)
          (:name "Sent (30d)"     :query "tag:sent AND date:30d.."            :key "s" :sort-order newest-first)
          (:name "Bulk (3d, FP check)" :query "tag:bulk AND date:3d.."        :key "b" :sort-order newest-first)
          ;; --- Customer / function views (work mail) ---------------------
          (:name "Leadership"     :query "tag:f/leadership AND tag:inbox"     :key "l")
          (:name "VAC"            :query "tag:c/VAC AND tag:inbox"            :key "v")
          (:name "Metro"          :query "tag:c/Metro AND tag:inbox"          :key "m")
          (:name "Partner"        :query "tag:f/partner AND tag:inbox"        :key "p")
          (:name "Sales"          :query "tag:f/sales AND tag:inbox"          :key "S")
          (:name "IGA"            :query "tag:t/IGA AND tag:inbox"            :key "a")
          (:name "AI"             :query "tag:t/AI AND tag:inbox"             :key "n"))))
;; Saved searches:1 ends here

;; [[file:config.org::*Aging colors][Aging colors:1]]
(after! notmuch
  (setq notmuch-search-line-faces
        '(("age/old"   . (:foreground "#bf616a" :weight bold))   ; red    — overdue
          ("age/7d"    . (:foreground "#d08770"))                ; orange — getting stale
          ("age/3d"    . (:foreground "#ebcb8b"))                ; yellow — keep an eye on
          ("age/today" . (:foreground "#a3be8c"))                ; green  — fresh
          ("owed/reply" . (:weight bold))))                      ; bold for action items

  ;; Suppress the aging tag pill on each line (the line color already shows it).
  ;; The owed/reply pill is kept so it visually distinguishes the two queues
  ;; when both appear in the same search.
  (setq notmuch-tag-formats
        '(("unread"     (propertize tag 'face '(:foreground "#88c0d0" :weight bold)))
          ("flagged"    (propertize tag 'face '(:foreground "#ebcb8b")))
          ("owed/reply" (propertize "↩" 'face '(:foreground "#bf616a" :weight bold)))
          ("s/waiting"  (propertize "⏳" 'face '(:foreground "#d08770")))
          ("age/today"  nil)
          ("age/3d"     nil)
          ("age/7d"     nil)
          ("age/old"    nil))))
;; Aging colors:1 ends here

;; [[file:config.org::*Hello dashboard][Hello dashboard:1]]
;; ---------- Faces ----------
;; Define stable face names so the dashboard's color scheme stays in sync
;; with `notmuch-search-line-faces`. If you switch themes, override these
;; faces, not the inline colors.
(defface my/notmuch-age-old   '((t (:foreground "#bf616a" :weight bold))) "Red — overdue (>=7d).")
(defface my/notmuch-age-7d    '((t (:foreground "#d08770")))               "Orange — getting stale (<7d).")
(defface my/notmuch-age-3d    '((t (:foreground "#ebcb8b")))               "Yellow — noticed (<3d).")
(defface my/notmuch-age-today '((t (:foreground "#a3be8c")))               "Green — fresh (<1d).")
(defface my/notmuch-dim       '((t (:inherit shadow)))                     "Dim accent text.")
(defface my/notmuch-section   '((t (:weight bold :inherit default)))       "Section header in hello.")

;; ---------- Helpers ----------
(defun my/notmuch--age-face (bucket count)
  "Face for COUNT in BUCKET (symbol: old, 7d, 3d, today). Dimmed at 0."
  (if (zerop count)
      'my/notmuch-dim
    (pcase bucket
      ('old   'my/notmuch-age-old)
      ('7d    'my/notmuch-age-7d)
      ('3d    'my/notmuch-age-3d)
      ('today 'my/notmuch-age-today)
      (_      'default))))

(defun my/notmuch--worst-face (n-old n-7d n-3d n-today)
  "Face for a queue row, taking the worst non-zero bucket."
  (cond ((> n-old 0)   'my/notmuch-age-old)
        ((> n-7d 0)    'my/notmuch-age-7d)
        ((> n-3d 0)    'my/notmuch-age-3d)
        ((> n-today 0) 'my/notmuch-age-today)
        (t             'my/notmuch-dim)))

(defun my/notmuch--open-query (button)
  (notmuch-search (button-get button 'notmuch-query)))

(defun my/notmuch--btn (label query face)
  "Insert LABEL as a clickable button opening QUERY in notmuch-search."
  (insert-text-button label
                      'face face
                      'notmuch-query query
                      'action #'my/notmuch--open-query
                      'follow-link t
                      'help-echo (format "notmuch-search %s" query)))

(defun my/notmuch--rule (width)
  (insert (propertize (make-string width ?─) 'face 'my/notmuch-dim))
  (insert "\n"))

(defun my/notmuch--count (query)
  "Synchronous single-query count. Used sparingly (header total only)."
  (string-to-number
   (replace-regexp-in-string
    "\n" "" (shell-command-to-string
             (format "notmuch count %s" (shell-quote-argument query))))))

;; ---------- Header ----------
(defun my/notmuch-hello-insert-header ()
  (let* ((total     (my/notmuch--count "*"))
         (sync-file "~/Library/Logs/mbsync.log")
         (sync-time (if (file-exists-p (expand-file-name sync-file))
                        (format-time-string
                         "%Y-%m-%d %H:%M"
                         (file-attribute-modification-time
                          (file-attributes (expand-file-name sync-file))))
                      "—")))
    (insert "\n  ")
    (insert (propertize "notmuch" 'face '(:weight bold :height 1.2)))
    (insert (propertize (format "   %s msgs   ·   last sync %s\n"
...
;; ---------- Action queues ----------
(defun my/notmuch-hello-insert-action-queue-row (glyph label base-query)
  "Insert one queue row with aging breakdown: total + 4 buckets, all clickable."
  (let* ((q-old   (format "%s AND tag:age/old"   base-query))
         (q-7d    (format "%s AND tag:age/7d"    base-query))
         (q-3d    (format "%s AND tag:age/3d"    base-query))
         (q-today (format "%s AND tag:age/today" base-query))
         (counts  (notmuch-hello-query-counts
                   (list (list :name "a" :query base-query)
                         (list :name "o" :query q-old)
                         (list :name "7" :query q-7d)
                         (list :name "3" :query q-3d)
                         (list :name "t" :query q-today))
                   :show-empty-searches t))
         (n-all   (plist-get (nth 0 counts) :count))
         (n-old   (plist-get (nth 1 counts) :count))
         (n-7d    (plist-get (nth 2 counts) :count))
         (n-3d    (plist-get (nth 3 counts) :count))
         (n-today (plist-get (nth 4 counts) :count))
         (worst   (my/notmuch--worst-face n-old n-7d n-3d n-today)))
    (insert "  ")
    (insert (propertize (concat glyph "  ") 'face worst))
    (my/notmuch--btn (format "%-12s" label) base-query worst)
    (insert (propertize (format " %4d   " n-all) 'face worst))
    (insert (propertize "old "   'face 'my/notmuch-dim))
    (my/notmuch--btn (format "%-2d" n-old)   q-old   (my/notmuch--age-face 'old   n-old))
    (insert (propertize "   7d "  'face 'my/notmuch-dim))
    (my/notmuch--btn (format "%-2d" n-7d)    q-7d    (my/notmuch--age-face '7d    n-7d))
    (insert (propertize "   3d "  'face 'my/notmuch-dim))
    (my/notmuch--btn (format "%-2d" n-3d)    q-3d    (my/notmuch--age-face '3d    n-3d))
    (insert (propertize "   today " 'face 'my/notmuch-dim))
    (my/notmuch--btn (format "%-2d" n-today) q-today (my/notmuch--age-face 'today n-today))
    (insert "\n")))

(defun my/notmuch-hello-insert-action-queues ()
  (insert "\n  ")
  (insert (propertize "Action queues" 'face 'my/notmuch-section))
  (insert (propertize "   ↩ owed / ⏳ waiting   ·   click any number\n"
                      'face 'my/notmuch-dim))
  (my/notmuch--rule 72)
  (my/notmuch-hello-insert-action-queue-row "↩" "Owed reply" "tag:owed/reply")
  (my/notmuch-hello-insert-action-queue-row "⏳" "Waiting"   "tag:s/waiting"))

;; ---------- Today's flow (deltas vs 03:00 snapshot) ----------
(defvar my/notmuch-hello-delta-tags
  '("inbox" "owed/reply" "s/waiting" "reading"
    "c/VAC" "c/Metro" "f/leadership" "bulk" "sent")
  "Tags surfaced in the daily delta line, in display order.")

(defun my/notmuch-hello--load-baseline ()
  "Return alist (TAG . COUNT) from today's snapshot, or nil if absent."
  (let* ((today (format-time-string "%Y-%m-%d"))
         (file  (expand-file-name
                 (format "~/org/notmuch-counts/%s.csv" today))))
...
(defun my/notmuch-hello-insert-deltas ()
  (let ((baseline (my/notmuch-hello--load-baseline)))
    (insert "\n  ")
    (insert (propertize "Today's flow" 'face 'my/notmuch-section))
    (cond
     ((not baseline)
      (insert (propertize "   (no baseline yet — first snapshot writes at 03:00)\n"
                          'face 'my/notmuch-dim))
      (my/notmuch--rule 72))
     (t
      (insert (propertize
               (format "   since %s 03:00 · click to open\n"
                       (format-time-string "%Y-%m-%d"))
               'face 'my/notmuch-dim))
      (my/notmuch--rule 72)
      (let* ((queries (mapcar (lambda (tag) (list :name tag :query (format "tag:%s" tag)))
                              my/notmuch-hello-delta-tags))
             (now (notmuch-hello-query-counts queries :show-empty-searches t)))
        (insert "  ")
        (dolist (entry now)
          (let* ((tag    (plist-get entry :name))
                 (n-now  (plist-get entry :count))
                 (n-base (or (cdr (assoc tag baseline)) 0))
                 (delta  (- n-now n-base))
                 (face   (cond ((zerop delta) 'my/notmuch-dim)
                               ((> delta 0)   'my/notmuch-age-today)
                               (t             'my/notmuch-age-7d))))
            (insert (propertize (format "%s " tag) 'face 'my/notmuch-dim))
            (my/notmuch--btn (format "%+d" delta) (format "tag:%s" tag) face)
            (insert "  ")))
        (insert "\n"))))))

;; ---------- Saved searches header ----------
(defun my/notmuch-hello-insert-saved-searches-header ()
  (insert "\n  ")
  (insert (propertize "Saved searches" 'face 'my/notmuch-section))
  (insert (propertize "   J <key> jumps · click name to open\n"
                      'face 'my/notmuch-dim))
  (my/notmuch--rule 72))
...
(after! notmuch
  (setq notmuch-hello-sections
        (list #'my/notmuch-hello-insert-header
              #'my/notmuch-hello-insert-action-queues
              #'my/notmuch-hello-insert-deltas
              #'my/notmuch-hello-insert-saved-searches-header
              #'notmuch-hello-insert-saved-searches
              #'notmuch-hello-insert-search
              #'notmuch-hello-insert-alltags)))
;; Hello dashboard:1 ends here

;; [[file:config.org::*Tag keybindings][Tag keybindings:1]]
(after! notmuch
  ;; Force K to mean notmuch-tag-jump in notmuch buffers, not Doom's docs lookup.
  (map! :map (notmuch-search-mode-map notmuch-show-mode-map notmuch-tree-mode-map)
        :n "K" #'notmuch-tag-jump)

  ;; All tag operations live here. Lower-case = add, upper-case = remove.
  ;; Customer/leadership tags are auto-applied by mail-sync rules; the entries
  ;; below exist so you can also apply them MANUALLY when the auto-rule misses
  ;; (forwarded customer mail, retroactive tagging, etc.).
  (setq notmuch-tagging-keys
        '(("a" notmuch-archive-tags        "Archive (-inbox)")
          ("u" notmuch-show-mark-read-tags "Mark read")
          ;; Workflow (transient — applied during triage, removed when resolved)
          ("w" ("+s/waiting")    "+ waiting")
          ("W" ("-s/waiting")    "- waiting")
          ("f" ("+s/follow-up")  "+ follow-up")
          ("F" ("-s/follow-up")  "- follow-up")
          ;; Customer (persistent — usually auto-applied, but manual override available)
          ("v" ("+c/VAC")        "+ VAC")
          ("V" ("-c/VAC")        "- VAC")
          ("m" ("+c/Metro")      "+ Metro")
          ("M" ("-c/Metro")      "- Metro")
          ;; Function (persistent)
          ("l" ("+f/leadership") "+ leadership")
          ("L" ("-f/leadership") "- leadership")
          ("p" ("+f/partner")    "+ partner")
          ("P" ("-f/partner")    "- partner")
          ("s" ("+f/sales")      "+ sales")
          ...
          ("n" ("+t/AI")         "+ AI")
          ("N" ("-t/AI")         "- AI"))))
;; Tag keybindings:1 ends here

;; [[file:config.org::*consult-notmuch — async live-preview search][consult-notmuch — async live-preview search:1]]
(use-package! consult-notmuch
  ...
  (map! :leader
        (:prefix-map ("s" . "search")
         :desc "notmuch (consult)" "m" #'consult-notmuch)))
;; consult-notmuch — async live-preview search:1 ends here

;; [[file:config.org::*Top-level bindings][Top-level bindings:1]]
(map! :leader
      (:prefix-map ("o" . "open")
